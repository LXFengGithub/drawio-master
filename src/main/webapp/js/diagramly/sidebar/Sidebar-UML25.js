(function()
{
	...
	Sidebar.prototype.addUml25Palette = function()
	{
		...
		var fns = [
			this.createVertexTemplateEntry('shape=umlFrame;whiteSpace=wrap;html=1;width=120;height=30;boundedLbl=1;verticalAlign=middle;align=left;spacingLeft=5;', 340, 180, 'Diagram1 heading', '图表', null, null, 'diagram'),
			this.createVertexTemplateEntry('shape=note2;boundedLbl=1;whiteSpace=wrap;html=1;size=25;verticalAlign=top;align=center;', 120, 60, 'Comment1 body', '评论的', null, null, 'uml comment note'),
			this.createVertexTemplateEntry('shape=partialRectangle;html=1;top=1;align=left;dashed=1;', 200, 20, 'Template1 signature', '模板的签名', null, null, 'template signature'),
			this.createVertexTemplateEntry('shape=partialRectangle;html=1;top=1;align=left;dashed=1;', 200, 50, 'Template parameter 1\nTemplate parameter 2', '模板的签名', null, null, 'template signature'),
			this.addEntry('constraint', function()
			{
				...
				return sb.createVertexTemplateFromCells([cell], cell.geometry.width, cell.geometry.height, '约束');
			}),
		 	this.addEntry(dt + 'classifier', function()
			{
				...
				return sb.createVertexTemplateFromCells([cell1], cell1.geometry.width, cell1.geometry.height, '分类器');
			}),
			this.createVertexTemplateEntry('shape=process2;fixedSize=1;size=5;fontStyle=1;', 140, 40, 'Classifier1', '分类器', null, null, 'classifier'),
		 	this.addEntry(dt + 'classifier', function()
			{
			...
			this.createVertexTemplateEntry('fontStyle=1;', 140, 30, 'Association1', '协会', null, null, 'association'),
		 	this.addEntry(dt + 'classifier', function()
			{
			...
			this.addEntry(dt + 'interface generalization', function()
			{
				...
				return sb.createEdgeTemplateFromCells([edge], 160, 0, '接口泛化');
			}),
			this.addEntry(dt + 'required interface', function()
			{
				...
				return sb.createEdgeTemplateFromCells([edge], 160, 0, '所需接口');
			}),
		 	this.addEntry(dt + 'property', function()
			{
				...
				return sb.createVertexTemplateFromCells([cell1], cell1.geometry.width, cell1.geometry.height, '财产');
			}),
			this.createVertexTemplateEntry('fontStyle=0;dashed=1;', 140, 30, 'Property1', '财产', null, null, 'property'),
			this.createVertexTemplateEntry('fontStyle=0;labelPosition=right;verticalLabelPosition=middle;align=left;verticalAlign=middle;spacingLeft=2;', 30, 30, 'port1', '港口', null, null, 'port'),
			this.addEntry(dt + 'port provided interface lollipop notation', function()
			{
				...
				return sb.createEdgeTemplateFromCells([cell1, edge1, cell2], 60, 30, '提供接口的端口');
			}),
